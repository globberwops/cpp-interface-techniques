...
using gw::cpp_interface_techniques::crtp::exceptions_map::MapAdapter;
...
TEST_F(CrtpExceptionsMapTest, RoadPredecessorsAndSuccessors) {
...
TEST_F(CrtpExceptionsMapTest, LaneNeighbors) {
  for (auto i = 0U; i <= 2; ++i) {
...
TEST_F(CrtpExceptionsMapTest, LaneLengthAndWidth) {
  for (auto i = 0U; i <= 2; ++i) {
    auto road = GetMap().GetRoad(RoadId{i});
    ASSERT_TRUE(road);
